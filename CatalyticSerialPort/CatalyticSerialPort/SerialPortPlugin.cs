        ...
        try
        {
            switch (action)
            {
                ...
                case CommAction.Send:
                    ...
                    if (!string.IsNullOrEmpty(options.CommandTerminator))
                        payloadToSend += options.CommandTerminator;
                    sendPort.Write(payloadToSend);
                    Service.AddPluginLog(Id, $"[Slot {slotIndex}] [{portName}] 已发送 {payloadToSend.Length} 字符");
                    break;
                ...
                case CommAction.Query:
                    ...
                    if (!string.IsNullOrEmpty(options.CommandTerminator))
                        queryPayload += options.CommandTerminator;
                    queryPort.Write(queryPayload);
                    Service.AddPluginLog(Id, $"[Slot {slotIndex}] [{portName}] 已发送 (Query) {queryPayload.Length} 字符");
                    ...
                    try
                    {
                        ...
                        lock (wrapper.Buffer)
                        {
                            ...
                            while (true)
                            {
                                ...
                                // 判定是否为“简单终止符”
                                bool isSimpleTerm = terminator == "\n" || terminator == "\r" || terminator == "\r\n" || terminator == "\t";

                                // 根据判定逻辑提取结果
                                string resultLine;
                                if (isSimpleTerm)
                                {
                                    resultLine = snapshot.Substring(0, termIdx);
                                }
                                else
                                {
                                    resultLine = snapshot.Substring(0, termIdx + terminator.Length);
                                }
                                ...
                                // 回推给引擎
                                _context?.PushEvent(slotIndex, portName, PluginEventType.Result, resultLine);
                            }
                        }
                    }
                    catch { }
