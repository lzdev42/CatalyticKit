...
public class InterceptorPlugin : IInterceptor
{
    ...
    {
        _context = context;
        _context.Log(LogLevel.Info, "拦截器插件已激活");
    ...
    {
        _context?.Log(LogLevel.Info, "拦截器插件已停用");
        return Task.CompletedTask;
    }
    ...
    {
        _context?.Log(LogLevel.Debug, $"[Slot {slotIndex}] 步骤执行前: {stepId} - {stepName}");
    ...
    {
        _context?.Log(LogLevel.Debug, $"[Slot {slotIndex}] 步骤执行后: {stepId} - {stepName} => {(passed ? "PASS" : "FAIL")}");
